  ...
  const fetchMentorshipData = async () => {
    try {
      ...
      if (match) {
        ...
        // Fetch mentor's extended profile separately
        const { data: mentorExtended } = await supabase
          .from('mentor_profiles')
          .select('headline, bio, expertise_areas, years_of_experience, linkedin_url, availability_status')
          .eq('user_id', match.mentor_id)
          .maybeSingle();

        // Combine the data - map to expected structure
        const combinedMentor = {
          id: mentorProfile?.id,
          full_name: mentorProfile?.full_name || 'Unknown Mentor',
          email: mentorProfile?.email,
          avatar_url: mentorProfile?.avatar_url,
          mentor_profiles: [{
            expertise: mentorExtended?.headline || mentorExtended?.expertise_areas?.join(', ') || 'General Mentorship',
            bio: mentorExtended?.bio || '',
            years_experience: mentorExtended?.years_of_experience || 0,
            linkedin_url: mentorExtended?.linkedin_url,
          }]
        };

        setMentorshipMatch({ ...match, mentor: combinedMentor });
        ...
        <CardContent className="pt-6">
          <div className="flex items-start gap-6">
            <Avatar className="h-20 w-20">
              <AvatarImage
                src={mentorshipMatch.mentor?.avatar_url}
                alt={mentorshipMatch.mentor?.full_name || 'Mentor'}
              />
              <AvatarFallback className="text-2xl bg-primary text-primary-foreground">
                {mentorshipMatch.mentor?.full_name?.charAt(0) || 'M'}
              </AvatarFallback>
            </Avatar>
            <div className="flex-1">
              <div className="flex items-start justify-between mb-2">
                <div>
                  <h3 className="text-2xl font-semibold">
                    {mentorshipMatch.mentor?.full_name || 'Loading...'}
                  </h3>
                  <p className="text-muted-foreground">
                    {mentorshipMatch.mentor?.mentor_profiles?.[0]?.expertise || 'Mentor'}
                  </p>
                </div>
              ...
              <p className="text-sm mb-4">
                {mentorshipMatch.mentor?.mentor_profiles?.[0]?.bio || 'No bio available'}
              </p>
              <div className="flex gap-4 text-sm text-muted-foreground">
                <span className="flex items-center gap-1">
                  <BookOpen className="h-4 w-4" />
                  {mentorshipMatch.mentor?.mentor_profiles?.[0]?.years_experience || 0} years experience
                </span>
                <span className="flex items-center gap-1">
